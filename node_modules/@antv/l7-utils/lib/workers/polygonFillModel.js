...
// src/workers/polygonFillModel.ts
var polygonFillModel_exports = {};
__export(polygonFillModel_exports, {
  polygonFillModel: () => polygonFillModel
});
module.exports = __toCommonJS(polygonFillModel_exports);
...
var polygonFillModel = async ({
  ...
  features.forEach((feature, featureIdx) => {
    const {
      ...
      size: vertexSize,
      indexes
    } = (0, import_triangulation.polygonFillTriangulation)(feature);
    ...
    for (let vertexIdx = 0; vertexIdx < verticesNumForCurrentFeature; vertexIdx++) {
      ...
      descriptors.forEach((descriptor) => {
        if (descriptor && updateFuncs[descriptor.name]) {
          descriptor.buffer.data.push(...updateFuncs[descriptor.name](feature, featureIdx, vertice, vertexIdx, normal, vertexIndex));
        }
      });
    }
  });
